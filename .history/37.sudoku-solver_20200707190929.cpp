...
public:
            ...
            for (; j < 9; j++)
            {
                if (board[i][j] == '.')
                {
                    ...
                    while (1)
                    {
                        ...
                        {
                            //printf("size:%d,%d,%d/n",candidateNum[candidateNum.size()-1]-48,i,j);
                            board[i][j] = candidateNum[candidateNum.size()-1];
                            candidateNum.pop_back();
                        }
                        if (fillNum(board, i+(j+1)/9, j+1-((j+1)/9)*9))
                        {
                            return true;
                        }
                    }
    ...
    vector<char> numCanUse(vector<vector<char>>& board, int i, int j){
        ...
        for (int ii = 0; ii < 9; ii++)
        {
            ...
            if (board[(i/3)*3 + ii/3][(j/3)*3 + ii-(ii/3)*3] != '.')
            {
                n[board[(i/3)*3 + ii/3][(j/3)*3 + ii-(ii/3)*3] - '0' - 1] = '0';
            }
        }
        vector<char> candidateNum;
        for (int ii = 0; ii < 9; ii++)
