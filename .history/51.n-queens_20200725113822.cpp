...
public:
    vector<vector<string>> solveNQueens(int n) {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            modifyBoard(0, j, n, chessboardtem);
            if (haveQ(1,0,n,chessboardtem,output))
            {
    ...
    bool haveQ(int i, int j, int n, vector<string>& chessboardtem,vector<vector<string>>& output)
    {
        string dot(n,'.');
        if (i==n)
        {
        ...
        else
        {
            for (i; i < n; i++)
            {
                for (j; j < n; j++)
                {
                    ...
                    //printf("i:%d,j:%d\n",i,j);
                    if (haveQ(i+1,0,n,chessboardtem1,output))
                    {
