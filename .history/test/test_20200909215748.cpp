...
int main() {
    ...
    priority_queue <int, greater<int>> value_heap;
    map<int,priority_queue <int, greater<int>>> mc;
    for (int i = 0; i < n; i++)
    {
        if (!mc.count(arr[i]))
        {
            priority_queue <int, greater<int>> tem;
            tem.push(i);
            mc[arr[i]] = tem;
        }
        else
    ...
    //
    map<int,priority_queue<int, greater<int>>>::iterator it;
    ...
    while (!value_heap.empty())
    {
        ...
        if ((int)mc[tem_value].size() >= 2)
        {
            ...
            {
                priority_queue<int, greater<int>> tem_vector;
