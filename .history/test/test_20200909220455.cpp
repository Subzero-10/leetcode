...
int main() {
    ...
    priority_queue<int, vector<int>, greater<int> > value_heap;
    map<int,priority_queue <int, vector<int>, greater<int> > > mc;
    for (int i = 0; i < n; i++)
    {
        if (!mc.count(arr[i]))
        {
    ...
    //
    map<int,priority_queueint<int, vector<int>, greater<int> > >::iterator it;
    ...
    while (!value_heap.empty())
    {
        ...
        if ((int)mc[tem_value].size() >= 2)
        {
            ...
            {
                priority_queue<int, vector<int>, greater<int> > tem_vector;
