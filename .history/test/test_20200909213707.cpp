...
int main() {
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        else
        {
            ...
            push_heap(mc[arr[i]].begin(),mc[arr[i]].end(), greater<int>());
        }
    }
    //
    ...
    for (it = mc.begin(); it != mc.end(); it++)
    {
        ...
        push_heap(value_heap.begin(),value_heap.end(), greater<int>());
    }
    while (!value_heap.empty())
    {
        ...
        {
            pop_heap(value_heap.begin(),value_heap.end(), greater<int>());
            int tem_index;
            tem_index = mc[tem_value][0];
            pop_heap(mc[tem_value].begin(),mc[tem_value].end(), greater<int>());
            arr[tem_index] = 0;
            int tem_index2;
            ...
            if (mc.count(tem_value*2))
            {
                ...
                push_heap(mc[tem_value*2].begin(), mc[tem_value*2].end(), greater<int>());
            }
            else
            {
