...
public:
    vector<vector<int>> insert(vector<vector<int>>& intervals, vector<int>& newInterval) {
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            if (situation == 2)
            {
                ...
                if (newright<nowleft)
                {
                    ...
                    intervals.erase(intervals.begin()+lastpos,intervals.begin()+i-1);
                    situation = 3;
                    break;
                }
            }
        ...
        case 1:
            ...
            intervals[len-1][0] = intervals[lastpos][0];
            if (lastpos!=(len-1))
            {
                intervals.erase(intervals.begin()+lastpos,intervals.end()-1);
            }
            break;
